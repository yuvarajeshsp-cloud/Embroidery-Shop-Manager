...
export function NewOrderPage() {
  ...
  React.useEffect(() => {
    supabase.from("customers").select("*").eq("archived", false).order("customer_business_name").then(({ data }) => {
      setCustomers(data || [])
    })
  ...
  const subtotal = items.reduce((s, i) => {
    const unit = i.manual_unit_price !== "" ? parseFloat(i.manual_unit_price) : (i.stitches_per_unit / 1000) * i.rate_per_1000_stitches
  ...
  async function handleSave() {
    ...
    try {
      ...
      const itemPayloads = items.map((item, idx) => ({
        ...
        stitches_per_unit: item.stitches_per_unit,
        rate_per_1000_stitches: item.rate_per_1000_stitches,
        manual_unit_price: item.manual_unit_price !== "" ? parseFloat(item.manual_unit_price) : null,
        setup_digitizing_charge: item.setup_digitizing_charge,
      ...
      await logAudit(profile, "orders", orderData.id, "INSERT", null, { order_number: orderForm.order_number } as Record<string, unknown>)
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Select
                ...
                onValueChange={(v) => setOrderForm({ ...orderForm, customer_id: v })}
              >
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                placeholder="e.g. WhatsApp, Walk-in, Phone"
              />
            </div>
            <div className="flex flex-col gap-2">
          ...
          <CardContent className="flex flex-col gap-4">
            {items.length === 0 ? (
              ...
              <div className="flex flex-col gap-4">
                {items.map((item, idx) => {
                  const calcPrice = (item.stitches_per_unit / 1000) * item.rate_per_1000_stitches
                      ...
                      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <Input
                            ...
                            min={0}
                            className="h-8"
                            value={item.stitches_per_unit}
                            onChange={(e) => updateItem(item.tempId, { stitches_per_unit: parseInt(e.target.value) || 0 })}
                          />
                        </div>
                        <div className="flex flex-col gap-1">
                          ...
                          <Input
                            ...
                            step="0.01"
                            className="h-8"
                            value={item.rate_per_1000_stitches}
                            onChange={(e) => updateItem(item.tempId, { rate_per_1000_stitches: parseFloat(e.target.value) || 0 })}
                          />
                        </div>
                        <div className="flex flex-col gap-1">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <Input
                            ...
                            step="0.01"
                            className="h-8"
                            value={item.setup_digitizing_charge}
                            onChange={(e) => updateItem(item.tempId, { setup_digitizing_charge: parseFloat(e.target.value) || 0 })}
                          />
                        </div>
                        <div className="flex flex-col gap-1">
